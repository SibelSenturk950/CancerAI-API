...
def predict_survival():
    ...
    try:
        ...
        # Validate gender-specific cancer types
        sex = data.get('sex')
        cancer_type = data.get('cancer_type')
        
        if sex == 'Male' and cancer_type == 'Ovarian Cancer':
            return jsonify({
                'error': 'Invalid combination',
                'message': 'Males cannot have ovarian cancer'
            }), 400
        
        if sex == 'Female' and cancer_type == 'Prostate Cancer':
            return jsonify({
                'error': 'Invalid combination',
                'message': 'Females cannot have prostate cancer'
            }), 400
